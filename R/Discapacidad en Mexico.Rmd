...
```{r}
mydata <- read.xlsx("Bases/cpv2020_b_eum_06_discapacidad.xlsx", 
                    sheet = "02", 
                    colNames = FALSE,
                    startRow = 10, 
                    skipEmptyRows = TRUE)
# Columnas
colnames(mydata) <- c("Entidad Federativa", "Sexo", "Grupos quinquenales", "Población Total", 
...
![](../Output/Piramide Población con discapacidad_Grupos de edad.png){width="90%" fig-align="center"} 
...
color_text <- c(rep("black", 18), rep("white", 14))
...
<p align="center">
  <img src="../Output/Población con discapacidad_Estados_Page_1.png" alt="Población con discapacidad" width="80%">
</p>
...
percent <- tabla %>% 
            adorn_percentages("col") %>%
             mutate(percent = .$Pob_Total *100)

# Asignar posiciones de burbujas
packing <- circleProgressiveLayout(tabla$Pob_Total, sizetype = 'area')
tabla <- cbind(tabla, packing)
...
fig <- plot_ly() %>%
        add_polygons(
                     ...
                     y = ~y,
                     split = ~id,
                     fillcolor = ~viridis(32, option = "A", begin = 0.3, end = 0.8, direction = -1)[id],
         ...
         add_trace(
                   ...
                   x = ~x, 
                   y = ~y,
                   marker = list(color = viridis(32, option = "A", begin = 0.3, end = 0.8, direction = -1)),
                   type = 'scatter',
                   hoverinfo = 'text',
                   mode="markers+text",
                   text = ~paste(Entidad, "\n", format(Pob_Total, big.mark=",")),
                   textfont = list(family = "Montserrat Medium", size = 13, color = c(rep("black", 18), rep("white", 14))),
                   hovertemplate = ~paste0(Entidad, "\n", format(Pob_Total, big.mark=","), "\n", round(percent$percent, 2), "%")
                  ) %>%
          layout(
            title = "Población con discapacidad por estado",
            showlegend = FALSE,
